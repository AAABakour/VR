...
public class ModernSimulationGameUI : MonoBehaviour
{
    ...
    public SimulationSelfTestSuite selfTestSuite;
    ...
    private TextMeshProUGUI performanceValue;
    ...
    private readonly List<UiActionBinding> uiActionBindings = new List<UiActionBinding>();
    private readonly List<string> registeredButtonLabels = new List<string>();
    ...
    private void AutoFindReferences()
    {
        ...
        if (selfTestSuite == null) selfTestSuite = Object.FindFirstObjectByType<SimulationSelfTestSuite>();
    }
    ...
    private void BuildUI()
    {
        ...
        ValidateDropdownTemplates(false);
    ...
    private void CreateTopBar(RectTransform parent)
    {
        ...
        performanceValue = CreateStatusChip("PERF --", bar, AccentGreen, 155f);
    ...
    private void CreateRightControlDeck(RectTransform parent)
    {
        ...
        CreatePerformanceAndQaSection(content);
    ...
    private void CreatePerformanceAndQaSection(RectTransform parent)
    {
        RectTransform section = CreateSection(parent, "FINAL QA + PERFORMANCE", "stable delivery profiles, low-resource mode and one-click diagnostics");

        RectTransform gridRoot = CreateUIObject("Performance Grid", section).GetComponent<RectTransform>();
        LayoutElement gridRootLayout = gridRoot.gameObject.AddComponent<LayoutElement>();
        gridRootLayout.preferredHeight = 240f;
        GridLayoutGroup grid = CreateGrid(gridRoot, 2, 42f);

        CreateButton("Balanced Final", gridRoot, () => ApplyPerformanceProfile(SimulationPerformanceOptimizer.PerformanceMode.FinalBalanced), 1, 42, AccentGreen, true);
        CreateButton("Low Resource", gridRoot, () => ApplyPerformanceProfile(SimulationPerformanceOptimizer.PerformanceMode.LowResource), 1, 42, AccentCyan, true);
        CreateButton("Presentation", gridRoot, () => ApplyPerformanceProfile(SimulationPerformanceOptimizer.PerformanceMode.PresentationQuality), 1, 42, AccentOrange, true);
        CreateButton("SPH Ready", gridRoot, () => ApplyPerformanceProfile(SimulationPerformanceOptimizer.PerformanceMode.SPHReady), 1, 42, new Color(0.73f, 0.30f, 1f, 1f), true);
        CreateButton("Ultra SPH Prep", gridRoot, () => ApplyPerformanceProfile(SimulationPerformanceOptimizer.PerformanceMode.UltraSPHPreparation), 1, 42, new Color(0.44f, 0.95f, 1f, 1f), true);
        CreateButton("Run QA Test", gridRoot, RunFinalQaSelfTest, 1, 42, AccentBlue, true);
        CreateButton("UI Smoke Test", gridRoot, RunUiSmokeTestFromButton, 1, 42, new Color(0.58f, 0.62f, 0.70f, 1f), true);

        grid.constraintCount = 2;
        LayoutElement layout = section.GetComponent<LayoutElement>();
        layout.preferredHeight = 328f;
    }
    ...
    private void CreateParticleSection(RectTransform parent)
    {
        ...
        CreateSlider(section, "Max Particles", 100f, 3000f,
            ...
            v => { if (particleSimulator != null) particleSimulator.SetMaxParticlesSafely(Mathf.RoundToInt(v)); }, "0", true);
    ...
    private void CreateSlider(RectTransform parent, string label, float min, float max, Func<float> getter, Action<float> setter, string format, bool wholeNumbers = false)
    {
        ...
        slider.onValueChanged.AddListener(v =>
        {
            ...
            RunControlSetterSafely(label, () => setter?.Invoke(v));
    ...
    private void CreateDropdown(RectTransform parent, string label, string[] options, Func<int> getter, Action<int> setter)
    {
        ...
        dropdown.onValueChanged.AddListener(i => RunControlSetterSafely(label, () => setter?.Invoke(i)));
    ...
    {
        // TMP_Dropdown is very strict: the template must be inactive, but the sample
        // item inside the template must remain ACTIVE and must contain a Toggle.
        // The previous version disabled the Item itself, which caused Unity to log:
        // "The dropdown template is not valid" whenever a dropdown was opened.
        ...
        template.sizeDelta = new Vector2(0f, Mathf.Max(132f, options.Length * 38f + 18f));
        ...
        content.pivot = new Vector2(0.5f, 1f);
        content.anchoredPosition = Vector2.zero;
        content.sizeDelta = new Vector2(0f, Mathf.Max(34f, options.Length * 36f));
        VerticalLayoutGroup contentLayout = content.gameObject.AddComponent<VerticalLayoutGroup>();
        ...
        if (dropdown != null)
        {
            dropdown.itemText = item.GetComponentInChildren<TextMeshProUGUI>(true);
    ...
    private Toggle CreateDropdownItem(RectTransform content, string name)
    {
        ...
        itemText.name = "Item Label";
        Stretch(itemText.rectTransform, 34f, 3f, 12f, 3f);
        return toggle;
    }
    ...
    private Button CreateButton(string label, RectTransform parent, Action action, float width, float height, Color accent, bool flexibleWidth = false)
    {
        ...
        registeredButtonLabels.Add(label);
        uiActionBindings.Add(new UiActionBinding(label, action));
        button.onClick.AddListener(() => RunUiActionSafely(label, action));
        return button;
    }

    private void RunControlSetterSafely(string label, Action action)
    {
        try
        {
            action?.Invoke();
        ...
        {
            lastUiMessage = label + " control failed: " + ex.GetType().Name;
    ...
    private void RunUiActionSafely(string label, Action action)
    {
        ...
        try
        {
            string previousMessage = lastUiMessage;
            action?.Invoke();
            if (string.IsNullOrEmpty(lastUiMessage) || lastUiMessage == previousMessage)
            {
    ...
    private void ApplyPerformanceProfile(SimulationPerformanceOptimizer.PerformanceMode mode)
    {
        if (performanceOptimizer == null)
        {
            performanceOptimizer = Object.FindFirstObjectByType<SimulationPerformanceOptimizer>();
        }

        if (performanceOptimizer != null)
        {
            performanceOptimizer.ApplyProfile(mode);
            lastUiMessage = performanceOptimizer.LastSummary;
        }
        else
        {
            lastUiMessage = "Performance optimizer not found";
        }
    }

    private void RunFinalQaSelfTest()
    {
        if (selfTestSuite == null)
        {
            selfTestSuite = Object.FindFirstObjectByType<SimulationSelfTestSuite>();
        }

        if (selfTestSuite != null)
        {
            lastUiMessage = selfTestSuite.RunFullSelfTestAndWriteReport();
        }
        else
        {
            lastUiMessage = "QA self-test suite not found";
        }
    }

    private void RunUiSmokeTestFromButton()
    {
        lastUiMessage = RunUiNonDestructiveSmokeTest();
    }

    public string RunUiNonDestructiveSmokeTest()
    {
        int total = 0;
        int missing = 0;
        int duplicated = 0;
        ...
        for (int i = 0; i < uiActionBindings.Count; i++)
        {
            UiActionBinding binding = uiActionBindings[i];
            if (binding == null)
            {
                continue;
            }
            ...
            total++;
            if (string.IsNullOrEmpty(binding.label) || binding.action == null)
            {
                missing++;
            }

            if (!string.IsNullOrEmpty(binding.label) && !seen.Add(binding.label))
            {
                duplicated++;
            }
        }

        int invalidDropdowns = ValidateDropdownTemplates(false);
        string result = missing == 0 && invalidDropdowns == 0 ? "PASS" : "WARN";
        return "UI Smoke Test " + result + ": " + total + " buttons registered, " + missing + " missing actions, " + duplicated + " duplicate labels, " + invalidDropdowns + " invalid dropdown templates";
    }

    private int ValidateDropdownTemplates(bool logDetails)
    {
        int invalid = 0;
        for (int i = 0; i < dropdownBindings.Count; i++)
        {
            DropdownBinding binding = dropdownBindings[i];
            if (binding == null || binding.dropdown == null || binding.dropdown.template == null)
            {
                invalid++;
                continue;
            }

            Toggle item = binding.dropdown.template.GetComponentInChildren<Toggle>(true);
            bool itemIsUsable = item != null && item.gameObject.activeSelf && item.transform.IsChildOf(binding.dropdown.template);
            if (!itemIsUsable || binding.dropdown.captionText == null || binding.dropdown.itemText == null)
            {
                invalid++;
                if (logDetails)
                {
                    Debug.LogWarning("Invalid TMP dropdown template repaired check failed on " + binding.dropdown.name);
                }
            }
        }
        return invalid;
    ...
    public int RegisteredButtonCount => registeredButtonLabels.Count;
    public int RegisteredSliderCount => sliderBindings.Count;
    public int RegisteredToggleCount => toggleBindings.Count;
    public int RegisteredDropdownCount => dropdownBindings.Count;

    public string GetRegisteredControlsSummary()
    {
        return "buttons=" + RegisteredButtonCount +
            ", sliders=" + RegisteredSliderCount +
            ", toggles=" + RegisteredToggleCount +
            ", dropdowns=" + RegisteredDropdownCount;
    }
    ...
    private void RefreshBindings()
    {
        for (int i = 0; i < sliderBindings.Count; i++)
        {
            ...
            float value;
            try { value = binding.getter(); }
            catch (Exception ex) { lastUiMessage = "Slider refresh failed: " + ex.GetType().Name; continue; }
        ...
        for (int i = 0; i < toggleBindings.Count; i++)
        {
            ...
            bool value;
            try { value = binding.getter(); }
            catch (Exception ex) { lastUiMessage = "Toggle refresh failed: " + ex.GetType().Name; continue; }
        ...
        for (int i = 0; i < dropdownBindings.Count; i++)
        {
            ...
            int value;
            try { value = Mathf.Clamp(binding.getter(), 0, Mathf.Max(0, binding.optionCount - 1)); }
            catch (Exception ex) { lastUiMessage = "Dropdown refresh failed: " + ex.GetType().Name; continue; }
    ...
    private void RefreshDashboard(bool force)
    {
        ...
        if (performanceOptimizer != null && performanceValue != null)
        {
            performanceValue.text = "PERF " + performanceOptimizer.CurrentMode;
        }
    ...
    private class UiActionBinding
    {
        public string label;
        public Action action;

        public UiActionBinding(string label, Action action)
        {
            this.label = label;
            this.action = action;
        }
