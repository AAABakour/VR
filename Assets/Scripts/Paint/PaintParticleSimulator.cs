...
public class PaintParticleSimulator : MonoBehaviour
{
    ...
    private void RebuildVisualPoolIfNeeded()
    {
        int currentSize = visualPool != null ? visualPool.Length : 0;
        int requestedSize = showVisualDroplets ? Mathf.Max(0, visualPoolSize) : 0;
        ...
        if (visualPool != null)
        {
            ...
            {
                if (visualPool[i] == null || visualPool[i].gameObject == null)
                {
                    continue;
                }
                ...
                {
                    Destroy(visualPool[i].gameObject);
                }
                else
                {
                    DestroyImmediate(visualPool[i].gameObject);
                }
            }
        }

        nextVisualSearchIndex = 0;
        InitializeVisualPool();
    }
